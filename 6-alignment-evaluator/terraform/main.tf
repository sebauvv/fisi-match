...
resource "aws_lambda_function" "alignment_evaluator" {
  ...
  runtime          = "python3.10"
  timeout          = 120 # Claude Haiku es rapido; 2 min es suficiente
  memory_size      = 256
...
resource "aws_iam_policy" "backend_invoke_policy" {
  ...
  description = "Permite al backend FastAPI invocar el Lambda de evaluacion de alineamiento"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Resource = aws_lambda_function.alignment_evaluator.arn
      }
    ]
  })
}
...
output "lambda_arn" {
  value = aws_lambda_function.alignment_evaluator.arn
}

output "lambda_function_name" {
  description = "Agregar como ALIGNMENT_LAMBDA_FUNCTION en el .env del backend"
  value       = aws_lambda_function.alignment_evaluator.function_name
}

output "lambda_region" {
  description = "Agregar como ALIGNMENT_LAMBDA_REGION en el .env del backend"
  value       = var.aws_region
}

output "backend_invoker_access_key_id" {
  description = "Agregar como ALIGNMENT_LAMBDA_ACCESS_KEY_ID en el .env del backend"
  value       = aws_iam_access_key.backend_invoker_key.id
}

output "backend_invoker_secret_access_key" {
  description = "Agregar como ALIGNMENT_LAMBDA_SECRET_ACCESS_KEY en el .env del backend"
