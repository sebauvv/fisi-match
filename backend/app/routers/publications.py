...
from app.schemas.publication import PublicationRead, PublicationListResponse
...
def list_publications(
    ...
    if search:
        query = query.where(ExternalPublication.title.ilike(f"%{search}%"))
    ...
    return PublicationListResponse(
        ...
        items=[PublicationRead(
            id=p.id, advisor_id=p.advisor_id, advisor_name=p.advisor_name,
            orcid=p.orcid, title=p.title, type=p.type, year=p.year,
            journal=p.journal, doi=p.doi, external_url=p.external_url,
        ) for p in pubs]
    )